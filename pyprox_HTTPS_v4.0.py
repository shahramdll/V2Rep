...
class DNS_over_Fragment:
    ...
    def query(self, server_name):
        ...
        try:
            ...
            ans = self.req.get(query_url, params=quary_params, headers={
                               'accept': 'application/dns-message'}, proxies=self.fragment_proxy)
            ...
            if ans.status_code == 200 and ans.headers.get('content-type') == 'application/dns-message':
                ...
                print(
                    f'online DNS --> Resolved {server_name} to {resolved_ip}')
                return resolved_ip
            else:
            ...
            try:
                if (first_flag == True):
                    ...
                    if data:
                        ...
                        IP_UL_traffic[this_ip] = IP_UL_traffic[this_ip] + \
                            len(data)
            ...
            try:
                if (first_flag == True):
                    ...
                    if data:
                        ...
                        IP_DL_traffic[this_ip] = IP_DL_traffic[this_ip] + \
                            len(data)
                    else:
...
def merge_all_dicts():
    # merge two dict , need python 3.5 or up
    full_DNS = {**DNS_cache, **active_Offline_DNS}
...
def log_writer():
    ...
    with open(log_file_path, "w") as f:
        while True:
            ...
            f.write('\n#############################################\n')
            f.write('\n########### ALL INFO : ######################\n')
            f.write(str(all_stats_info).replace(
                '\'', '').replace(',', '\n').replace(':', '\t'))
            f.write('\n#############################################\n')
            f.flush()
            f.truncate()
